...
def toStars(command, user):
    ...
    dest_path='%s%s/%s/%s'%(stars_image_path, datetime.datetime.utcnow().strftime('%Y'), datetime.datetime.utcnow().strftime('%Y-%m-%d'),'itzamna')
    #send_message(dest_path)
    #off they go!
    (output, error, pid) = runSubprocess(['tostars','%s*.fits'%image_path,'%s'%dest_path])
    if error == '':
        send_message('Successfully uploaded %d image(s) to <http://stars.uchicago.edu/fitsview17/|stars>!'%len(fits))
...
def doImage(command, user):
    ...
    fits = image_path + '%s_%s_%ssec_bin%s_%s_%s_num%d_seo.fits'%(target_name, filter, exposure, binning, 'mcnowinski', datetime.datetime.utcnow().strftime('%Y%b%d_%Hh%Mm%Ss.%f'), 0)
    fits = fits.replace(' ', '_')
...
def doFocus(command, user):
    ...
    if(match):
        ...
        (output, error, pid) = runSubprocess(['tx','focus', 'pos=%s'%position])    
        #send_message(output)
        #done focus pos=4854
...
def getClearDarkSky(command, user):
    ...
    dummy = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(5))
    send_message("", [{"image_url":"http://www.cleardarksky.com/c/SonomaCAcsk.gif?dummy=%s"%dummy, "title":"Lake Sonoma Clear Sky Chart"}])
    send_message("", [{"image_url":"http://www.cleardarksky.com/c/SmnCAcsk.gif?dummy=%s"%dummy, "title":"Sonoma Clear Sky Chart"}])
    send_message("\n")    
        ...
        if 'type' in msg and 'channel' in msg and 'user' in msg:
            if msg['type'] == 'message' and msg['channel'] == slack_channel:
                ...
                if user == None: #make a fake one
                    ...
                    user = {'name' : 'Slack User', 'profile' : {'display_name' : 'Slack User', 'email' : 'user@slack.com', 'phone' : '8675309'}}
                ...
                if(dt > dt_last_message):
                    ...
                    if match:
                        ...
                        try:
                            logme('User %s sent text (%s) on %s.'%(user['profile']['display_name'], msg['text'],dt_last_message.strftime("%Y/%m/%d @ %H:%M:%S")))
                        except:
                            pass
                    #is this a command, starts with \      
                else:
                    logme('Warning! Ignoring old/duplicate message from #%s ("%s" from %s).'%(slack_channel_name,msg['text'], user['profile']['display_name']))         
...
def parse_command(text, user, dt):
    ...
    for command in commands:
        ...
        if match: #compare with list of known commands, spelling and capitalization count!
            logme('%s sent command (%s).'%(user['profile']['display_name'], text))
            #call associated function
            command[1](text, user)
            break;
        #index += 1
    if not match: #did not recognize this command
        logme('%s sent unrecognized command (%s) on %s.'%(user['profile']['display_name'], text, dt.strftime("%Y/%m/%d @ %H:%M:%S")))
        send_message('%s, the almighty Itzamna does not recognize your command (%s).'%(user['profile']['display_name'],text))  
